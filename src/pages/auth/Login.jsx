    ...
    <div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <button
          disabled={loading}
          type="submit"
          className={`w-full rounded bg-red-600 py-3 font-semibold text-white transition-colors flex items-center justify-center ${loading ? "opacity-70 cursor-not-allowed" : "hover:bg-red-700"}`}
        >
          {loading ? <SpinnerButton /> : "Masuk"}
        </button>
      </form>

      <p className="mt-6 text-center text-sm text-gray-500">
        belum punya akun? registrasi{" "}
        <Link to="/auth/register" className="font-bold text-red-600">
          di sini
        </Link>
      </p>
    </div>
