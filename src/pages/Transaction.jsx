...
export default function Transaction() {
  ...
  useEffect(() => {
    async function fetchData() {
      setLoading(true);
      try {
        const transactionResponse = await apiClient(`/transaction/history?offset=${offset}&limit=${limit}`, "GET", null, token);
        
        // Jika offset 0, ganti data. Jika lebih, tambahkan ke daftar yang ada
        setTransactions((prev) => offset === 0 ? transactionResponse.data.records : [...prev, ...transactionResponse.data.records]);
      } catch (error) {
        console.error(error);
      } finally {
        setLoading(false);
        ...
        <div className="space-y-4">
          {transactions.map((t) => (
            <div key={t.id} className="flex items-center justify-between p-5 border rounded-lg border-gray-200">
              <div>
                <h4 className={`text-2xl font-semibold ${t.transaction_type === "TOPUP" ? "text-green-500" : "text-red-500"}`}>
                  {t.transaction_type === "TOPUP" ? "+" : "-"} Rp{t.total_amount.toLocaleString('id-ID')}
                </h4>
